...
function labelStatus(status) {
  return { PENDING: 'Chờ xử lý', PASSED: 'Đỗ', FAILED: 'Trượt' }[status] || status || '';
}

function labelShift(shift) {
  return { MORNING: 'Ca sáng', AFTERNOON: 'Ca chiều' }[shift] || shift || '';
}

function labelGender(gender) {
  return { MALE: 'Nam', FEMALE: 'Nữ' }[gender] || gender || '';
}
...
function canManageInterviews(user) {
  return isAdmin(user) || normalizeAccountType(user?.account_type, user?.role) === 'QUAN_LY';
}
...
function canApproveLeaves(user) {
  return ['DEPUTY_MANAGER', 'DEPARTMENT_HEAD', 'ADMIN'].includes(normalizeRole(user?.role));
}
...
function getDepartmentName(user) {
  return String(user?.department_name || user?.department || '').trim();
}
...
app.put('/api/users/:id', requireAdmin, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (password) {
      const hash = await bcrypt.hash(password, 10);
      await client.query(
        `UPDATE users SET username=$2, email=$3, full_name=$4, role=$5, account_type=$6, is_active=$7, employment_start_date=$8, annual_leave_manual_adjustment=$9, password_hash=$10, password=$11, updated_at = NOW() WHERE id=$1`,
        [id, username, email, fullName, role, accountType, isActive, employmentStartDate, annualAdjustment, hash, password]
      );
    } else {
      await client.query(
        `UPDATE users SET username=$2, email=$3, full_name=$4, role=$5, account_type=$6, is_active=$7, employment_start_date=$8, annual_leave_manual_adjustment=$9, updated_at = NOW() WHERE id=$1`,
...
app.get('/api/stats', requireAuth, asyncHandler(async (req, res) => {
  ...
  const getInterviewCount = async (built) => {
    ...
    const r = await pool.query(
      `SELECT COUNT(*)::int AS total,
              COALESCE(SUM(CASE WHEN i.status='PENDING' THEN 1 ELSE 0 END),0)::int AS pending,
              COALESCE(SUM(CASE WHEN i.status='PASSED' THEN 1 ELSE 0 END),0)::int AS passed,
...
app.post('/api/leave', requireAuth, leaveUpload.single('attachment'), asyncHandler(async (req, res) => {
  ...
  if (leaveType === 'Nghỉ phép' && annualLeaveDaysUsed <= 0) {
    return res.status(400).json({ error: 'Nếu chọn nghỉ phép thì phải nhập số phép năm sử dụng.' });
  }
  ...
  if (!isAdminUser) {
    if (!approverId) return res.status(400).json({ error: 'Vui lòng chọn người ký duyệt.' });

    approver = await getUserById(approverId);
    if (!approver) return res.status(404).json({ error: 'Không tìm thấy người ký.' });

    if (getRoleLevel(approver.role) <= getRoleLevel(currentUser.role)) {
      return res.status(400).json({ error: 'Người ký phải có cấp cao hơn người tạo đơn.' });
    }
    ...
    if (!sameDept && normalizeRole(approver.role) !== 'ADMIN') {
      return res.status(400).json({ error: 'Người ký phải cùng phòng ban hoặc là Admin.' });
    }
  } else {
    status = 'approved';
    approvedAt = new Date();
  ...
  if (annualLeaveDaysUsed > 0) {
    ...
    if (annualLeaveDaysUsed > balance.remaining_days) {
      return res.status(400).json({ error: `Phép năm còn lại không đủ. Hiện còn ${balance.remaining_days} phép.` });
    }
  }

  const startParts = req.body.start_time
    ? timePartsFromValue(req.body.start_time)
    : { hour: req.body.start_hour || null, minute: req.body.start_minute || null, time: buildTimeValue(req.body.start_hour, req.body.start_minute) };

  const endParts = req.body.end_time
    ? timePartsFromValue(req.body.end_time)
    : { hour: req.body.end_hour || null, minute: req.body.end_minute || null, time: buildTimeValue(req.body.end_hour, req.body.end_minute) };
  ...
  const { rows } = await pool.query(
    `INSERT INTO leave_requests(
      user_id, requester_role, requester_department_id, approver_id, approver_role,
      leave_type, reason, from_date, to_date,
      start_hour, start_minute, end_hour, end_minute,
      start_time, end_time, annual_leave_days_used,
      file_name, file_path, status, approved_at, created_at, updated_at
    )
    VALUES(
      $1,$2,$3,$4,$5,
      $6,$7,$8,$9,
    ...
    [
      ...
      approver ? normalizeRole(approver.role) : null,
      leaveType,
      reason,
      fromDate,
      toDate,
...
async function approveOrRejectLeave(req, res, action) {
  ...
  try {
    ...
    if (Number(leave.approver_id || 0) !== Number(req.session.user.id)) {
      await client.query('ROLLBACK');
      return res.status(403).json({ error: 'Bạn không có quyền xử lý đơn này.' });
    }
...
app.get('/api/leaves/:id/download', requireAuth, asyncHandler(async (req, res) => {
  ...
  const canAccess =
    ...
    Number(leave.user_id) === Number(req.session.user.id) ||
    Number(leave.approver_id) === Number(req.session.user.id) ||
    (canViewDepartment(req.session.user) && safeLower(leave.department_name) === safeLower(getDepartmentName(req.session.user)));
...
app.post('/api/interviews', requireAuth, imageUpload.fields([{ name: 'cccd_front', maxCount: 1 }, { name: 'cccd_back', maxCount: 1 }]), asyncHandler(async (req, res) => {
  ...
  for (const item of [interviewDate, birthDate, cccdIssueDate]) {
    if (!isValidDateInput(item)) return res.status(400).json({ error: 'Có ngày tháng không hợp lệ.' });
  }
  ...
  const { rows } = await pool.query(
    ...
    [
      ...
      phone,
      gender,
      front ? front.buffer : null,
      front ? front.mimetype : null,
...
app.get('/api/interviews/export.xlsx', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!built.empty) {
    ...
    rows.forEach((row) => {
      sheet.addRow({
        ...row,
        interview_shift: labelShift(row.interview_shift),
        gender: labelGender(row.gender),
...
app.get('/api/logs', requireAdmin, asyncHandler(async (req, res) => {
  const { rows } = await pool.query(
    `SELECT l.id, l.action, l.old_status, l.new_status, l.note, l.created_at, u.full_name AS user_name, i.full_name AS worker_name
     FROM interview_logs l
     LEFT JOIN users u ON u.id = l.user_id
     LEFT JOIN interview_forms i ON i.id = l.form_id
     ORDER BY l.id DESC
     LIMIT 200`
  );
  res.json({ logs: rows });
